...
test("spacing autofix preserves comments and is stable across files", () => {
  const probeRoot = mkdtempSync(join(tmpdir(), "blindfolded-spacing-"));
  ...
  function runSpacingLint(...args: string[]) {
    const result = spawnSync("bun", ["run", "oxlint", "--config", config, ...args], {
      encoding: "utf8",
    });
  ...
  try {
    writeFileSync(
      ...
      JSON.stringify({
        jsPlugins: [
          { name: "blindfolded", specifier: fileURLToPath(new URL("../index.ts", import.meta.url)) },
        ],
        rules: { "blindfolded/require-readable-spacing": "error" },
      }),
    );
