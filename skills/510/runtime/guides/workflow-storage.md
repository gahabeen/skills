# Store specs and debugging evidence
...
The result includes absolute `specs` and `debug` directories. It reads the existing
...
| Storage choice | Specs | Debug sessions |
| --- | --- | --- |
| Project (default) | `.510/specs/` | `.510/debug/` |
| Shared or custom | `<base>/projects/<project-id>/specs/` | `<base>/projects/<project-id>/debug/` |
...
Invoking `510 spec`, `510 implement`, or `510 debug` authorizes creating that workflow's local
...
Project-local specs are durable documents: setup does not add them to its generated
...
existing specs or debug sessions.
